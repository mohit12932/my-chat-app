...
const Page = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      let response = await axios.post(`${process.env.NEXT_PUBLIC_SERVER_URL}/signin`, data, {
      ...
      if (response.status == 201) {
        ...
        setTimeout(() => router.push('/chatpage'), 800);
      }
    } catch (error) {
          ...
          {success ? (
            ...
            <form className="w-full max-w-md" onSubmit={handleSubmit(onSubmit)}>
              ...
              <div className="flex items-center justify-center mt-6">
                <a href="#" className="w-1/3 pb-2 font-medium text-center capitalize border-b-2 border-blue-400">sign in</a>
                <a href="./SignUp" className="w-1/3 pb-2 font-medium text-center capitalize border-b">sign up</a>
              </div>
              ...
              <div className="mt-4">
                ...
                <input type="text" {...register("emailaddress", { required: 'Email address is required' })} onChange={() => clearErrors('existUser')}
              ...
              <div className="mt-8">
                <button 
                  ...
                  className={`w-full px-6 py-2.5 text-sm font-medium tracking-wide capitalize transition-colors duration-300 transform border rounded-lg focus:outline-none focus:ring focus:ring-gray-300 focus:ring-opacity-50 ${
                    isSubmitting
                      ? 'bg-gray-500 text-white cursor-not-allowed opacity-75' 
                      : 'bg-transparent hover:text-gray-800 hover:bg-white'
                  }`}
                >
